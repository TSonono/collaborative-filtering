...
/**
 * Converts an array to a hash map with keys according to the index of the
 * elements.
 * @param {array} arr
 * @returns {map} A map with the elements from the array mapped to indices.
 */
 ...
 *        For each rated item:
 *            In the (normalized) co-occurrence matrix, sum each row (column-wise) to a new array
...
function getRecommendations(ratings, coMatrix, userIndex) {
  ...
  // Sum each column in the similarity matrix to an array
  ...
  // Then divide each column by numRatedItems
  recommendations = math.dotDivide(recommendations, numRatedItems);
  console.log(recommendations);

  // TODO: Sort the resulting array in descending order and keep track of index...
